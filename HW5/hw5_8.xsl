...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/ReviewApplication">
        ...
        <table border="1">
            ...
            <xsl:variable name="res" select="'restaurantid1'"></xsl:variable>
            <xsl:for-each select="/ReviewApplication/Reviews/Review[RestaurantId/text()=$res]">
            <xsl:variable name="restid" select="RestaurantId"></xsl:variable>
                <tr>
                    <td>
